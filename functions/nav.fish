function nav -d "Navigational Assistance with Velocity"
  ...
  # Parse flags
  if argparse -n nav -x (string join -- ' -x ' h,w,t,f,b,n,p,s,r,m,l,a,x,d e,c {e,c},{h,s,r,m,l,a,x,d} | string split ' ') 'h/help' 'w/where' 't/to' 'f/foward' 'b/back' 'p/prevd' 'n/nextd' 's/save' 'r/remove' 'm/move' 'l/list' 'a/autoremove' 'x/abbr' 'd/backup' 'e/echo' 'c/commander' 'y/assume-yes' -- $argv 2>&1 | read err
    err $err
  ...
  if string match -qr -- '[trmla]' $flags
    if not find $_nav_bookmarks -type l 2>/dev/null | string length -q
      ...
      source $functions/instructions.fish "nav -s/--save"
      return 1
    end
  end
  ...
  if test $status = 0
    source $_nav_subfunctions/instructions.fish "nav -"(string match -vr '[ec]' $flags)"/--\S+"
    return 1
  end
